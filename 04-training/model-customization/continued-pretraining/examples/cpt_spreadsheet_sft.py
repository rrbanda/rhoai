#!/usr/bin/env python3
"""Continued pretraining on Excel spreadsheets using standard SFT.

Downloads the SpreadsheetBench dataset, converts ``.xlsx`` files to markdown
text via ``markitdown``, then runs SFT with ``is_pretraining=True`` for
continued pretraining.  Unlike the OSFT variant, all parameters are updated
with no orthogonal subspace constraints.

Adapted from the Training Hub ``sft_cpt_spreadsheet_example``.
...
Usage:
    ...
    python cpt_spreadsheet_sft.py --prepare-only
"""
...
# ---------------------------------------------------------------------------
# Data preparation (identical to the OSFT variant)
# ---------------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Continued pretraining on Excel spreadsheets with SFT.",
    ...
    parser.add_argument("--max-tokens-per-gpu", type=int, default=25000, help="Tokens per GPU per step (default: 25000)")
    parser.add_argument("--batch-size", type=int, default=64, help="Effective batch size (default: 64)")
    parser.add_argument("--learning-rate", type=float, default=2e-6, help="Learning rate (default: 2e-6)")
...
def main() -> None:
    ...
    print()
    print("=" * 60)
    print("SFT Continued Pretraining: Excel Spreadsheets")
    print("=" * 60)
