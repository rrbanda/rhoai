#!/usr/bin/env python3
"""Train Qwen 2.5 7B Instruct with SFT or OSFT.

Supports both standard supervised fine-tuning and OSFT (Orthogonal Subspace
Fine-Tuning) for continual learning without catastrophic forgetting.

Adapted from the Training Hub ``sft_qwen_example`` and ``osft_qwen_example``.

Hardware requirements:
    8x A100 40 GB recommended (SFT: 20k tokens/GPU, OSFT: 16k tokens/GPU).
...
Usage:
    ...
    python train_qwen.py --algorithm osft \\
        --data-path /path/to/data.jsonl \\
        --ckpt-output-dir /path/to/checkpoints \\
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Qwen 2.5 7B Instruct")
    ...
    parser.add_argument(
        "--max-tokens-per-gpu", type=int, default=None, help="Tokens/GPU (auto by algorithm)"
    )
...
def main() -> None:
    ...
    defaults = {
        "sft": {"max_tokens_per_gpu": 20000, "learning_rate": 1e-5},
    ...
    print("=" * 60)
    print(f"{algo} Training: Qwen 2.5 7B Instruct")
    print("=" * 60)
