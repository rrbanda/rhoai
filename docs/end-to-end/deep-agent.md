...
graph TB
    ...
    agent -->|"port-forward :8000"| vllm
    agent -->|"HTTP :8009"| mcp
    agent --- memory
    agent --- skills
```

The agent runs locally and calls the fine-tuned model on the cluster via `oc port-forward`. The MCP server provides 15 financial tools. In production, both the agent and MCP server would run as containers on the same cluster.
...
=== "Tab 2: Port-Forward to vLLM"

    ```bash
    oc port-forward \
      -n tool-calling-financial \
      deployment/tool-calling-financial-lora-predictor \
      8000:8080
    ```
    ...
    You should see both `financial-agent-lora` (base) and `financial-agent` (LoRA adapter).
...
The agent uses the **fine-tuned LoRA adapter** (`financial-agent`), not the raw base model. The fine-tuning:
...
```bash
# Shows both base model and LoRA adapter
...
```
financial-agent-lora           root=/mnt/models  parent=(base)
financial-agent                root=/mnt/lora-adapter/output  parent=financial-agent-lora
```
...
| `Connection refused` on port 8000 | `oc port-forward` may have timed out. Restart it |
