#!/usr/bin/env python3
"""OSFT Training with MLflow Experiment Tracking.

Demonstrates Orthogonal Subspace Fine-Tuning (OSFT) with MLflow integration
using Training Hub on Red Hat OpenShift AI. OSFT preserves the model's
general capabilities by constraining updates to an orthogonal subspace,
making it ideal for domain-specific tuning without catastrophic forgetting.

MLflow automatically logs training parameters, metrics, and model artifacts.
...
Example usage:
    # Basic:
    python osft_with_mlflow.py --data-path ./train_data.jsonl

    # With custom OSFT rank ratio:
    python osft_with_mlflow.py \\
        --data-path ./train_data.jsonl \\
        --unfreeze-rank-ratio 0.15

    # Multi-GPU with custom MLflow experiment:
    python osft_with_mlflow.py \\
        --data-path ./train_data.jsonl \\
        --nproc-per-node 4 \\
        --mlflow-experiment my-osft-experiment \\
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="OSFT Training with MLflow Experiment Tracking",
    ...
    model_group.add_argument(
        "--ckpt-output-dir",
        default=os.getenv("CKPT_OUTPUT_DIR", "./osft-mlflow-checkpoints"),
        help="Directory for model checkpoints (env: CKPT_OUTPUT_DIR)",
    )

    osft_group = parser.add_argument_group("OSFT Parameters")
    osft_group.add_argument(
        "--unfreeze-rank-ratio",
        type=float,
        default=0.1,
        help="Ratio of ranks to unfreeze in orthogonal subspace (0.0-1.0)",
    )
    ...
    mlflow_group.add_argument(
        "--mlflow-experiment",
        default=os.getenv("MLFLOW_EXPERIMENT_NAME", "osft-training"),
...
def main() -> None:
    ...
    print("=" * 60)
    print("OSFT Training with MLflow Experiment Tracking")
    print("=" * 60)
